...
int gridSize;
PImage stone, ice, trampoline, spike, bridge, treeIntersect, treeMiddle, treeEndWest, treeEndEast, treeTrunk, walk1, walk2, jump, goomba, thwomp, thwompEvil, hammerBro, hammer, hammerBroReversed, portal, movingPlatform;
...
void setup() {
  ...
  movingPlatform=loadImage("movingBlock.png");
  movingPlatform.resize(gridSize*5, gridSize);
    ...
    for (int y=0; y<map.height; y++) {
      for (int x=0; x<map.width; x++) {
        ...
        if (c==black) {
          ...
          //println(foundX+" "+foundY);
          if(foundX && foundY){
            FMovingPlatform m=new FMovingPlatform(x*gridSize, y*gridSize, tempX*gridSize, tempY*gridSize);
            m.setName("mover");
            terrain.add(m);
            world.add(m);
          }
          else if(foundX){
            FMovingPlatform m=new FMovingPlatform(x*gridSize, y*gridSize, tempX*gridSize, y*gridSize);
            m.setName("mover");
            terrain.add(m);
            world.add(m);
          }
          else{
            FMovingPlatform m=new FMovingPlatform(x*gridSize, y*gridSize, x*gridSize, tempY*gridSize);
            m.setName("mover");
            terrain.add(m);
            world.add(m);
          }
